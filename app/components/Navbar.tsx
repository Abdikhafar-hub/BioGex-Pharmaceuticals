...
export default function Navbar() {
  ...
  return (
    <>
      {/* Navbar */}
      <nav
        className={`fixed  top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-[#2e7d32] shadow-lg" : "bg-transparent"
          }`}
      >
        {/* inset div black */}
        {/* <div className="absolute inset-0 bg-black opacity-10 z-10" /> */}
        <div className="max-w-9xl mx-auto px-4 sm:px-6 lg:px-8 z-40">
          <div className="flex justify-between items-center h-16">
            ...
            <div className={`hidden md:block`}>
              <div className="ml-10 flex items-baseline space-x-2">
                {[
                  ...
                  { name: "Get a Quote", id: "quote", isCTA: true }
                ].map((item, index) => (
                  <button
                    key={item.name}
                    onClick={() => scrollToSection(item.id)}
                    className={`relative px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 group overflow-hidden ${isScrolled ? "text-white hover:text-white" : "text-white hover:text-white"
                      } ${item.isCTA ? "bg-[#2e7d32] text-white hover:scale-105 hover:shadow-xl hover:shadow-[#2e7d32]/30" : ""}
                      ${item.isCTA && isScrolled ? "bg-white !text-[#2e7d32] hover:scale-105 hover:shadow-xl hover:shadow-[#2e7d32]/30" : ""}`}
            ...
            <div className="md:hidden">
              <button
                ...
                className="p-2 rounded-md text-white bg-black/10 hover:bg-black/20 backdrop-blur-sm transition-colors"
              >
      ...
      >
        ...
        {/* Mobile Menu Panel */}
        <div
          className={`absolute top-0 right-0 h-full w-80 bg-gradient-to-b from-[#2e7d32] via-[#1b5e20] to-[#2e7d32] shadow-2xl transform transition-all duration-300 ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"
            }`}
        >
          {/* Header */}
